...
static av_cold int h264_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->extradata_size > 0 && avctx->extradata) {
		av_log(NULL, AV_LOG_INFO, "h264 ppt, in h264_decode_init, go to ff_h264_decode_extradata.\n");
                             ...
                             int *got_frame, AVPacket *avpkt)
{
    ...
    if (h->is_avc && av_packet_get_side_data(avpkt, AV_PKT_DATA_NEW_EXTRADATA, NULL)) {
        ...
        if (is_extra(side, side_size)){
			av_log(NULL, AV_LOG_INFO, "h264 ppt, in h264_decode_frame, go to ff_h264_decode_extradata 1.\n");
    ...
    if(h->is_avc && buf_size >= 9 && buf[0]==1 && buf[2]==0 && (buf[4]&0xFC)==0xFC && (buf[5]&0x1F) && buf[8]==0x67){
        if (is_extra(buf, buf_size)){
			av_log(NULL, AV_LOG_INFO, "h264 ppt, in h264_decode_frame, go to ff_h264_decode_extradata 2.\n");

                             ...
                             int err_recognition, void *logctx)
{
    ...
    if (!data || size <= 0)
        return -1;
	av_log(logctx, AV_LOG_INFO, "h264 ppt, in ff_h264_decode_extradata, data:\n", size);
    for(i = 0; i < size; i++)
    {
        av_log(logctx, AV_LOG_INFO, "%2x", data[i]);
    }
	av_log(logctx, AV_LOG_INFO, "\n");
    if (data[0] == 1) {
        int i, cnt, nalsize;
        const uint8_t *p = data;
		av_log(logctx, AV_LOG_INFO, "h264 ppt, in ff_h264_decode_extradata, data[0]: 1.\n");
        ...
        *nal_length_size = (data[4] & 0x03) + 1;
		...
		av_log(logctx, AV_LOG_INFO, "h264 ppt, in ff_h264_decode_extradata, data[0]: %d.\n", (int)data[0]);
        *is_avc = 0;

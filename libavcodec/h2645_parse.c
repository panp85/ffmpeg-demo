    ...
    while (si + 2 < length) {
        ...
        if (src[si + 2] > 3) {
            ...
            if (src[si + 2] == 3) { // escape
                ...
                av_log(NULL, AV_LOG_INFO, "h2645 ppt, in ff_h2645_extract_rbsp, go to nsc.\n");
                goto nsc;
            }
        }
		av_log(NULL, AV_LOG_INFO, "h2645 ppt, in ff_h2645_extract_rbsp, dst[di++] = src[si++].\n");
        dst[di++] = src[si++];
    }
    while (si < length)
        dst[di++] = src[si++];
...
static int find_next_start_code(const uint8_t *buf, const uint8_t *next_avc)
{
	...
	av_log(NULL, AV_LOG_INFO, "h2645 ppt, in find_next_start_code, i = %d.\n", i);
    return i + 3;
}
                          ...
                          enum AVCodecID codec_id, int small_padding)
{
    ...
    while (bytestream2_get_bytes_left(&bc) >= 4) {
        ...
        if (consumed < 0)
            return consumed;
		av_log(NULL, AV_LOG_INFO, "h2645 ppt, in ff_h2645_packet_split, consumed = %d.\n", consumed);
		...
		av_log(NULL, AV_LOG_INFO, "h2645 ppt, in ff_h2645_packet_split,2 bc.buffer: %2x%2x%2x.\n", bc.buffer[0], bc.buffer[1], bc.buffer[2]);

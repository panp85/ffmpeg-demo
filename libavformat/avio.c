...
static const struct URLProtocol *url_find_protocol(const char *filename)
{
    ...
    if ((ptr = strchr(proto_nested, '+')))
        *ptr = '\0';
	av_log(NULL, AV_LOG_WARNING, "ffmpeg ppt, in url_find_protocol, proto_nested: %s, filename: %s.\n", proto_nested, filename);
                ...
                const AVIOInterruptCB *int_cb)
{
    ...
    if (p){
	   av_log(NULL, AV_LOG_WARNING, "ffmpeg ppt, in ffurl_alloc, p: %s.\n", p->name);
                         ...
                         URLContext *parent)
{
    AVDictionary *tmp_opts = NULL;
    AVDictionaryEntry *e;
	av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in ffurl_open_whitelist, filename: %s.\n", filename);

...
//在rtpproto.c中会有区别发送到rtp端口还是rtcp端口
void ff_rtp_send_punch_packets(URLContext *rtp_handle)
{
    AVIOContext *pb;
    uint8_t *buf;
    int len;
    av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in ff_rtp_send_punch_packets, rtp_handle->prot->name: %s.\n", 
		rtp_handle->prot->name);
                                    ...
                                    const char *attr, const char *value))
{
    ...
    while (ff_rtsp_next_attr_and_value(&p,
                                       attr, sizeof(attr),
                                       value, value_size)) {
        av_log(s, AV_LOG_INFO, "rtp ppt, in ff_parse_fmtp, go to parse_fmtp, attr, value: %s, %s.\n",
			attr, value);

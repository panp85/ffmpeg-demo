...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    av_log(NULL, AV_LOG_INFO, "mux ppt, in init_muxer, s->nb_streams = %d.\n", s->nb_streams);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->oformat->init) {
		av_log(NULL, AV_LOG_INFO, "ffmpet ppt, in init_muxer, s->oformat->name: %s, go to s->oformat->init.\n", s->oformat->name);
...
static int compute_muxer_pkt_fields(AVFormatContext *s, AVStream *st, AVPacket *pkt)
{
	...
	av_log(s, AV_LOG_INFO, "compute_muxer_pkt_fields: pts:%s dts:%s cur_dts:%s b:%d size:%d st:%d, pkt->duration: %d.\n",
				av_ts2str(pkt->pts), av_ts2str(pkt->dts), av_ts2str(st->cur_dts), delay, pkt->size, pkt->stream_index, 
				pkt->duration);
...
static int do_packet_auto_bsf(AVFormatContext *s, AVPacket *pkt) {
    ...
    //av_log(NULL, AV_LOG_INFO, "mux ppt, in do_packet_auto_bsf, st->internal->nb_bsfcs = %d.\n", st->internal->nb_bsfcs);

...
static void build_chunks(MOVTrack *trk)
{
    ...
    for (i = 1; i<trk->entry; i++){
        ...
        if (chunk->pos + chunkSize == trk->cluster[i].pos &&
			...
			av_log(NULL, AV_LOG_INFO, "mov ppt, in build_chunks, cluster[%d].chunkNum = %d, trk->chunkCount = %d.\n", 
				i, chunk->chunkNum+1, trk->chunkCount);
...
static int mov_setup_track_ids(MOVMuxContext *mov, AVFormatContext *s)
{
    ...
    if (mov->track_ids_ok)
        return 0;
	av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_setup_track_ids, mov->use_stream_ids_as_track_ids = %d.\n",
					mov->use_stream_ids_as_track_ids);
    ...
    if (mov->use_stream_ids_as_track_ids) {
        ...
        for (i = 0; i < mov->nb_streams; i++) {
			av_log(NULL, AV_LOG_INFO, 
				"mov ppt, in mov_setup_track_ids, mov->tracks[%d].entry, mov->tracks[%d].entry = %d.\n",
				i, i, mov->tracks[i].entry);
            if (mov->tracks[i].entry <= 0 && !(mov->flags & FF_MOV_FLAG_FRAGMENT))
                continue;
                              ...
                              AVFormatContext *s)
{
    ...
    av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_moov_tag, go in, pos = %lld.\n", pos);
	...
	av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_moov_tag, mov->chapter_track = %d.\n",
	    mov->chapter_track);
	...
	av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_moov_tag, mov->iods_skip = %d.\n", mov->iods_skip);
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (ret < 0)
        return ret;
    av_log(NULL, AV_LOG_INFO, "mov ppt, in ff_mov_write_packet, mov->flags = %x.\n", mov->flags);
    if (mov->flags & FF_MOV_FLAG_FRAGMENT) {
        int ret;
...
static int mov_init(AVFormatContext *s)
{
    ...
    av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in mov_init, mov->flags = 0x%x.\n", mov->flags);
    if (mov->flags & FF_MOV_FLAG_DELAY_MOOV)
	...
	av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in mov_init, over mov->flags = 0x%x.\n", mov->flags);
	...
	av_log(NULL,AV_LOG_INFO, "mov ppt, in mov_init, s->nb_streams = %d.\n", s->nb_streams);
    for (i = 0; i < s->nb_streams; i++) {
        AVStream *st= s->streams[i];
        MOVTrack *track= &mov->tracks[i];
        AVDictionaryEntry *lang = av_dict_get(st->metadata, "language", NULL,0);
		...
		av_log(NULL,AV_LOG_INFO, "mov ppt, in mov_init, st->codecpar->codec_type = %d.\n", st->codecpar->codec_type);
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
			av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_init, track->tag = 0x%x.\n", track->tag);
...
static int mov_write_header(AVFormatContext *s)
{
	...
	av_log(NULL, AV_LOG_INFO, 
 		"mov ppt, in mov_write_header, mov->mode & MODE_MP4|MODE_MOV|MODE_IPOD: 0x%x, s->nb_chapters = %d, mov->flags = 0x%x.\n", 
 		mov->mode & (MODE_MP4|MODE_MOV|MODE_IPOD), s->nb_chapters, mov->flags);
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        /* copy extradata if it exists */
		av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_header, st->codecpar->extradata_size: %d.\n",
        ...
        if (st->codecpar->extradata_size) {
            if (st->codecpar->codec_id == AV_CODEC_ID_DVD_SUBTITLE)
				...
				av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_header, extradata:\n");
				for(j = 0; j < track->vos_len; j++)
				{
				    av_log(NULL, AV_LOG_INFO, "%02x ", st->codecpar->extradata[j]);
				}
				av_log(NULL, AV_LOG_INFO, "\n");
    ...
    if (mov->reserved_moov_size){
		av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_header, mov->reserved_moov_size = %d.\n", 
			mov->reserved_moov_size);
    ...
    if (mov->chapter_track){
		av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_header, mov->chapter_track yes.\n");
...
static int mov_write_trailer(AVFormatContext *s)
{
    ...
    if (!(mov->flags & FF_MOV_FLAG_FRAGMENT)) {
        ...
        av_log(NULL, AV_LOG_INFO, "mov ppt, in mov_write_trailer, go to mov_write_moov_tag.\n");
        if (mov->flags & FF_MOV_FLAG_FASTSTART) {

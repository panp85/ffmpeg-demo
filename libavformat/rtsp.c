                       ...
                       int payload_type, const char *line)
{
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        ...
        if (rtsp_st->sdp_payload_type == payload_type &&
            ...
            av_log(NULL, AV_LOG_INFO, "rtsp ppt, in parse_fmtp, go to parse_sdp_a_line.\n");
                           ...
                           int letter, const char *buf)
{
	...
	av_log(s, AV_LOG_INFO, "rtsp ppt, sdp: %c='%s'\n", letter, buf);
    ...
    case 'a':
        if (av_strstart(p, "control:", &p)) {
            ...
            s1->seen_rtpmap = 1;
            if (s1->seen_fmtp) {
				av_log(NULL, AV_LOG_INFO, "rtsp ppt, in sdp_parse_line, rtpmap, payload_type = %d.\n",
					payload_type);
            ...
            payload_type = atoi(buf1);
            if (s1->seen_rtpmap) {
				av_log(NULL, AV_LOG_INFO, "rtsp ppt, in sdp_parse_line, fmtp/framesize, payload_type = %d.\n",
					payload_type);
            ...
            if (s->nb_streams > 0) {
                ...
                if (rtsp_st->dynamic_handler &&
                    rtsp_st->dynamic_handler->parse_sdp_a_line)
                {
                    av_log(NULL, AV_LOG_INFO, "rtsp ppt, in sdp_parse_line, go to parse_sdp_a_line.\n");

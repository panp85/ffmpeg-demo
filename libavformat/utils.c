...
static int64_t wrap_timestamp(const AVStream *st, int64_t timestamp)
{
    av_log(NULL, AV_LOG_INFO, 
		"ffmpeg ppt, in wrap_timestamp, st->pts_wrap_reference = %llx, timestamp = %d, st->pts_wrap_behavior = %d.\n", 
		st->pts_wrap_reference, timestamp, st->pts_wrap_behavior);
                      ...
                      AVDictionary **options)
{
    ...
    av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in init_input, filename: %s.\n", filename);
    if (s->pb) {
        s->flags |= AVFMT_FLAG_CUSTOM_IO;
		av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in init_input, s->pb ok.\n");
        if (!s->iformat)
	...
	av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in init_input, 2.\n");

    if ((s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format2(&pd, 0, &score))))
        return score;
	av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in init_input, 3, filename: %s.\n", filename);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (s->pb){
		av_log(NULL, AV_LOG_ERROR, "ppt, in avformat_open_input, go to ff_id3v2_read_dict.\n");
        ff_id3v2_read_dict(s->pb, &s->internal->id3v2_meta, ID3v2_DEFAULT_MAGIC, &id3v2_extra_meta);
    }
	else{
		av_log(NULL, AV_LOG_ERROR, "ppt, in avformat_open_input, s->pb no.\n");
	}

    if (!(s->flags&AVFMT_FLAG_PRIV_OPT) && s->iformat->read_header){
        av_log(NULL, AV_LOG_INFO, 
			"ffmpeg ppt, in avformat_open_input, go to s->iformat->read_header, iformat: %s, url: %s.\n", 
			s->iformat->name, s->url);
...
{
    av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in probe_codec, st->request_probe = %d.\n", st->request_probe);
...
static int update_wrap_reference(AVFormatContext *s, AVStream *st, int stream_index, AVPacket *pkt)
{
    ...
    av_log(NULL, AV_LOG_ERROR, 
		"ppt, in update_wrap_reference, ref: %llx,  st->pts_wrap_reference: %llx, pts_wrap_bits: %d, correct_ts_overflow = %x.\n", 
		ref, st->pts_wrap_reference, st->pts_wrap_bits, s->correct_ts_overflow);
...
int ff_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        pkt->size = 0;
        av_init_packet(pkt);
		av_log(NULL,AV_LOG_INFO, "ffmpeg ppt, in ff_read_packet, s->iformat: %s.\n", s->iformat->long_name);
        ...
        st = s->streams[pkt->stream_index];
        //av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in ff_read_packet, st->pts_wrap_behavior = %d.\n", st->pts_wrap_behavior);
		...
		av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in ff_read_packet, pkt->stream_index: %d.\n", pkt->stream_index);
        ...
        {
            av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in ff_read_packet, use_wallclock_as_timestamps yes.\n");    
                               ...
                               int64_t next_dts, int64_t next_pts)
{
    ...
    int onein_oneout = st->codecpar->codec_id != AV_CODEC_ID_H264 &&
                       st->codecpar->codec_id != AV_CODEC_ID_HEVC;
    av_log(NULL, AV_LOG_INFO, "ffmpet ppt, in compute_pkt_fields, s->flags = %x.\n", s->flags);
...
static int parse_packet(AVFormatContext *s, AVPacket *pkt, int stream_index)
{
    ...
    while (size > 0 || (pkt == &flush_pkt && got_output)) {
        ...
        av_log(NULL, AV_LOG_INFO, "parser ppt, in parse_packet, out_pkt.pos,pts,dts: %lld, %lld, %lld.\n", 
			out_pkt.pos, out_pkt.pts, out_pkt.dts);
        ...
        if (st->parser->key_frame == -1 && st->parser->pict_type ==AV_PICTURE_TYPE_NONE && (pkt->flags&AV_PKT_FLAG_KEY))
            out_pkt.flags |= AV_PKT_FLAG_KEY;
        av_log(NULL, AV_LOG_INFO, "parse ppt, in parse_packet, out_pkt.flags = %x, out_pkt.duration: %d.\n", 
            out_pkt.flags, out_pkt.duration);
...
static int read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while (!got_packet && !s->internal->parse_queue) {
        ...
        if (st->internal->need_context_update) {
			av_log(NULL, AV_LOG_INFO, "ppt, in read_frame_internal, need_context_update yes.\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        av_log(NULL, AV_LOG_INFO,"ppt, in read_frame_internal, pkt->duration: %d.\n", 
            pkt->duration);

        if (st->need_parsing && !st->parser && !(s->flags & AVFMT_FLAG_NOPARSE)) {
			av_log(NULL, AV_LOG_INFO, "ffmpet ppt, in read_frame_internal, st->codecpar->codec_id = %d, go to av_parser_init.\n", 
...
#define FAIL(errmsg) do {                                         \
		...
		av_log(NULL, AV_LOG_ERROR, "ppt, in has_codec_parameters, failed: %s.\n", errmsg); \
		return 0;                                                 \
    } while (0)
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (st->info->found_decoder >= 0 && avctx->pix_fmt == AV_PIX_FMT_NONE)
			...
			av_log(NULL, AV_LOG_ERROR, "ppt, in has_codec_parameters, avctx->pix_fmt: %d.\n", avctx->pix_fmt);
		}
...
static int extract_extradata(AVStream *st, AVPacket *pkt)
{
    ...
    AVPacket *pkt_ref;
    int ret;
    av_log(NULL, AV_LOG_INFO, "ppt, in extract_extradata, go in.\n");
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE) && st->request_probe <= 0) {
			av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in avformat_find_stream_info, go to av_parser_init.\n");
            st->parser = av_parser_init(st->codecpar->codec_id);
            if (st->parser) {
        ...
        if (!has_codec_parameters(st, NULL) && st->request_probe <= 0) {
            if (codec && !avctx->codec){
				av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in avformat_find_stream_info, go to avcodec_open2 2.\n");
                if (avcodec_open2(avctx, codec, options ? &options[i] : &thread_opt) < 0)
                    av_log(ic, AV_LOG_WARNING,
                           "Failed to open codec in %s\n",__FUNCTION__);
            }
    ...
    for (;;) {
        ...
        for (i = 0; i < ic->nb_streams; i++) {
            ...
            {
                av_log(NULL, AV_LOG_ERROR, 
					"ppt, in avformat_find_stream_info, has_codec_parameters no, st->id: %d, codec_id: %d.\n", 
		...
		*/
        ...
        if (st->codec_info_nb_frames>1) {
            ...
            if (pkt->duration) {
                av_log(NULL, AV_LOG_INFO, "ppt, in avformat_find_stream_info, pkt->duration: %d.\n",
                    pkt->duration);
         ...
         * the container. */
         av_log(NULL, AV_LOG_INFO, "ppt, in avformat_find_stream_info, go to try_decode_frame.\n");
	...
	av_log(NULL, AV_LOG_INFO, "ppt, in avformat_find_stream_info, st->codec_info_nb_frames: %d.\n", st->codec_info_nb_frames);
    if (eof_reached) {
        int stream_index;
...
FF_ENABLE_DEPRECATION_WARNINGS
	...
	av_log(NULL, AV_LOG_INFO, "ffmpeg ppt, in avpriv_set_pts_info, new_tb: %d, %d.\n", new_tb.den, new_tb.num);
...
int ff_parse_creation_time_metadata(AVFormatContext *s, int64_t *timestamp, int return_seconds)
{
    ...
    if ((entry = av_dict_get(s->metadata, "creation_time", NULL, 0))) {
		av_log(NULL, AV_LOG_INFO, "ffmpet ppt, in ff_parse_creation_time_metadata, creation_time yes.\n");
